...
<body>
    ...
    <main>
        ...
        <div class="container marketing">
            <div class="display-3">Fasilitas Umum <span class="text-muted">Hotel Tacenda</span></div>
            <hr style="margin-bottom: 55px">
            {{-- <div class="offset-md-11 mt-4 mb-2">
                <a class="btn btn-primary" href="{{ route('tamu') }}">Kembali</a>
            </div> --}}
            <div class="row row-cols-1 row-cols-md-2 g-4">
                @foreach ($hotel as $i => $hotell)
                <div class="col">
                    <div class="card">
                        <img src="{{ url('/fasilitas_hotel/') . '/' . $hotell->gambar }}" class="card-img-top ini" alt="{{ $hotell->gambar }}">
                        {{-- <img src="{{ asset('img/deluxe.jpg') }}" class="card-img-top ini" alt="{{ $hotell->gambar }}"> --}}
                        <div class="card-body">
                            <h5 class="card-title">{{ $hotell->nama_fasilitas }}</h5>
                            <p class="card-text">{{ $hotell->keterangan }}</p>
                        </div>
                    </div>
                </div>
                @endforeach
